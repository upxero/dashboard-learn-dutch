...
function DropdownFilter({ align, setActiveFilters }) {
  ...
  const Checkrefs = {
    Dutch: useRef(null),
    Flemish: useRef(null),
    Exampreps: useRef(null),
  };

  const handleFilters = () => {
    // Clear all active filters when 'Clear' is clicked
  ...
  const handleCheckboxChange = () => {
    const activeFilters = [];
    if (Checkrefs.Dutch.current.checked) activeFilters.push('Dutch');
    if (Checkrefs.Flemish.current.checked) activeFilters.push('Flemish');
    if (Checkrefs.Exampreps.current.checked) activeFilters.push('Exampreps');

    setActiveFilters(activeFilters); // Update filters state
  };
          ...
          <ul className="mb-4">
            <li className="py-1 px-3">
              <label className="flex items-center">
                ...
                />
                <span className="text-sm font-medium ml-2">Dutch</span>
              </label>
            </li>
            <li className="py-1 px-3">
              <label className="flex items-center">
                <input
                  ref={Checkrefs.Flemish}
                  type="checkbox"
                  className="form-checkbox"
                  onChange={handleCheckboxChange}
                />
                <span className="text-sm font-medium ml-2">Flemish</span>
              </label>
            </li>
            <li className="py-1 px-3">
              <label className="flex items-center">
                <input
                  ref={Checkrefs.Exampreps}
                  type="checkbox"
                  className="form-checkbox"
                  onChange={handleCheckboxChange}
                />
                <span className="text-sm font-medium ml-2">Exampreps</span>
              </label>
            </li>
          </ul>
